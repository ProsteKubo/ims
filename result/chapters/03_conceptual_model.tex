...
\begin{tikzpicture}[node distance=3cm,>=stealth',bend angle=45,auto]
    ...
    \tikzstyle{transition}=[rectangle,thick,draw=black!75,fill=black!20,minimum width=8mm,minimum height=8mm]
    
    \node [place,tokens=2] (pain) {$P_1$};
    \node [place] (relief) [right of=pain] {$P_2$};
    \node [place,tokens=1] (motiv) [below of=pain] {$P_3$};
    \node [place,tokens=1] (alive) [below of=relief] {$P_6$};
    
    \node [transition] (assess) [right of=motiv] {$T_1$} 
        edge [pre] (alive)
        edge [post] (pain)
        edge [post] (relief);
    
    \node [transition] (increase) [below of=motiv] {$T_2$}
        edge [pre] (pain)
        edge [pre] (motiv)
        edge [post,bend right] (relief);
    
    \node [transition] (toxic) [right of=alive] {$T_4$}
        edge [pre] (alive);
    
    \draw [->] (assess) to [bend left] node[above] {measure $E$} (relief);
    \draw [->] (increase) to [bend right] node[below] {$A \mathrel{+{=}} D$} (motiv);
    
    \node at (0,-4.5) {\small Pain Level};
    \node at (2.5,-4.5) {\small Relief};
    \node at (0,-2.5) {\small Motivation};
    \node at (2.5,-2.5) {\small Alive};
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=2.5cm, auto, thick]
    ...
    \tikzstyle{cloud} = [ellipse, draw, fill=red!20, text width=5em, text centered, minimum height=3em]
    
    \node [block] (cont) {Continuous\\ODEs};
    \node [block, right of=cont, node distance=5cm] (disc) {Discrete\\Petri Net};
    \node [cloud, below of=cont, node distance=3cm] (sat) {Saturation\\Zone};
    
    \draw [->] (cont) -- node[above] {$E(t), C(t)$} (disc);
    \draw [->] (disc) -- node[below] {$D_i \cdot \delta(t-t_i)$} (cont);
    \draw [->] (cont) -- node[left] {$C \approx K_m$} (sat);
    \draw [->, red, thick] (sat) to [bend left=60] node[right] {Runaway} (cont);
\end{tikzpicture}
